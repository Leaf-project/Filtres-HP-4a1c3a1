	...
	uint zero_accumulation_tous_les[C])
{
	...
	FOR(0, i, I) {
		...
		mdl_aller_retour(mdl, t0, t1, 3, MODE_t_MODE, grain_t_MODE);
...
#define optimiser_la_couche zeroiser
		...
		//
		if (i % /*5*/100 == 0) {
			float* __pred = mdl_pred(mdl, t0, t1, 3, MODE_t_MODE, grain_t_MODE);
			float  _score = mdl_score(mdl, t0, t1, 3, MODE_t_MODE, grain_t_MODE);
			//
